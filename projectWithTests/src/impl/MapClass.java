...
public class MapClass implements Runnable {
    ...
    public MapClass(int id, Long start, Long end) {
        ...
        this.word_index_i = -1;
        this.word_index_j = -1;
        this.searchquery_count = new HashMap<String, Long>();
    }
    ...
    public void run() {
        ...
        while (counter < total) {
            ...
            // check if the word matches with any of the first words in the searchquery sub-names
            // if it does identify potential match searchquery name
            string_word = curr_word.toString();
            if (!find_word_index(string_word)) continue;   
            else  potential_match = Master.getInstance().searchquery_array.get(word_index_i).get(word_index_j);

            // if there are more words in the searchquery sub-name, then check next bytes in the file
            ...
            // check if the string matches expected searchquery name
            final_string = new ArrayList<String>(Arrays.asList(curr_word.toString().split(" ")));
            if (arrays_equal(final_string, potential_match) == -1) {
                continue;
            }

            // if there is a full match, icrement searchquery count in a local HashMap
        ...
        // write locally stored searchquery and their respective counts into memory
        this.wirteSearchqueryCountsToCSV();
    ...
    // increments searchquery count in a local hash map
    private void incremenetCountLocally(List<String> searchquery){
        String hash_name = "";
        ...
        if (curr_value == null) {
            searchquery_count.put(hash_name, 1L);
        } else {
            searchquery_count.put(hash_name, curr_value + 1L);
        }
    }

    // writes the local hash map results into memory csv file
    private void wirteSearchqueryCountsToCSV(){
         try {
    ...
    // if a word is contianed in the searchquery list then set word indeces to it
